...
def get_speed_limit(actor):
    town = CarlaDataProvider.get_map().name.split('/')[-1]
    file_content = np.load(f"{os.environ['SCENARIO_RUNNER_ROOT']}/speed_limits/{town}_speed_limits.npy", allow_pickle=True)
    ...
    idx = np.linalg.norm(map_locations - pos, axis=-1).argmin()
    return map_speed_limits[idx]
    ...
    def update(self):
        ...
        if self._parameters is not None:
            ...
            if "max_speed" in self._parameters:
                ...
                max_road_speed = get_speed_limit(self._actor)
...
class WaypointWalkerWithTimeout(AtomicBehavior):

    def __init__(self, ai, locations, timeout, center_loc=None, max_dist=None, name="Idle"):
        """
        Setup actor
        """
        ...
        super(WaypointWalkerWithTimeout, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))

    def initialise(self):
        super(WaypointWalkerWithTimeout, self).initialise()
        self.last_time = GameTime.get_time()
        self.ai.start()
        self.ai.go_to_location(self.locations[self.loc_idx])

    def update(self):
        tgt_loc = self.locations[self.loc_idx]

        if not self.ai.is_alive or not self.ai.parent.is_alive:
            return py_trees.common.Status.FAILURE

        act_loc = self.ai.parent.get_location()
        if act_loc.distance(tgt_loc) < 2 or GameTime.get_time() - self.last_time > self.timeout or act_loc.distance(self.center_loc) > self.max_dist:
            self.loc_idx += 1
            if self.loc_idx >= len(self.locations):
                return py_trees.common.Status.SUCCESS

            self.last_time = GameTime.get_time()
            self.ai.go_to_location(tgt_loc)
...
class OppositeActorFlow(AtomicBehavior):
    ...
    def initialise(self):
        ...
        self._speed = get_speed_limit(self._reference_actor) # Km / h
